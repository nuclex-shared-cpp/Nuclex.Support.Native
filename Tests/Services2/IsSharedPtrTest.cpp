...
namespace Nuclex::Support::Services2::Private {
  ...
  TEST(IsSharedPtrTest, IntegerIsNotASharedPtr) {
    EXPECT_FALSE(IsSharedPtr<int>::value);
  }

  // ------------------------------------------------------------------------------------------- //

  TEST(IsSharedPtrTest, PlainPointerIsNotASharedPtr) {
    EXPECT_FALSE(IsSharedPtr<float *>::value);
  }

  // ------------------------------------------------------------------------------------------- //

  TEST(IsSharedPtrTest, AnySharedPtrIsDetected) {
    EXPECT_TRUE(IsSharedPtr<std::shared_ptr<int>>::value);
    EXPECT_TRUE(IsSharedPtr<std::shared_ptr<const float>>::value);
  }
