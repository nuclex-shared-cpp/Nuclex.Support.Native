...
#ifndef NUCLEX_SUPPORT_COLLECTIONS_MULTIMAP_H
#define NUCLEX_SUPPORT_COLLECTIONS_MULTIMAP_H
...
namespace Nuclex { namespace Support { namespace Collections {
  ...
  /// <summary>Interface for a container that maps (duplicate) keys to values</summary>
  /// <typeparam name="TKey">Type of the key the multi-map uses</typeparam>
  /// <typeparam name="TValue">Type of values that are stored in the map</typeparam>
  template<typename TKey, typename TValue>
  class MultiMap {

    /// <summary>Destroys the multi-map</summary>
    public: virtual ~MultiMap() = default;
    ...
    /// <remarks>
    ///   If the multi-map contains the key more than once, the value of an arbitrary
    ///   key will be returned. Specific multi-map implementations may provide guarantees
    ///   that the first or oldest value will be returned, but other implementations,
    ///   such as caches, may just return an arbitrary or the easiest to reach value
    ///   out of the values stored under the requested key.
    /// </remarks>
    ...
    /// <param name="key">Key of the elements that will be removed if present</param>
    /// <returns>The number of elements that were removed</returns>
    public: virtual std::size_t TryRemove(const TKey &key) = 0;
...
#endif // NUCLEX_SUPPORT_COLLECTIONS_MULTIMAP_H
