...
namespace Nuclex { namespace Support { namespace Text {
  ...
  template<> void lexical_append<>(std::string &target, const std::uint8_t &from) {
    ...
    ::itoa_fwd(from, target.data() + length);
  }
  ...
  template<> void lexical_append<>(std::string &target, const std::int32_t &from) {
    ...
    if(from >= 1) {
      ...
      ::itoa_fwd(static_cast<std::uint32_t>(from), target.data() + length);
    } else if(from == 0) {
      target.push_back('0');
    } else {
  ...
  template<> std::size_t lexical_append<>(
    ...
    if(from >= 1) {
      ...
      if(availableBytes >= requiredBytes) {
        ::itoa_fwd(static_cast<std::uint32_t>(from), target);
      }
      return requiredBytes;
    } else if(from == 0) {
  ...
  template<> void lexical_append<>(std::string &target, const std::int64_t &from) {
    ...
    if(from >= 1) {
      ...
      ::itoa_fwd(static_cast<std::uint64_t>(from), target.data() + length);
    } else if(from == 0) {
      target.push_back('0');
    } else {
  ...
  template<> std::size_t lexical_append<>(
    ...
    if(from >= 1) {
      ...
      if(availableBytes >= requiredBytes) {
        ::itoa_fwd(static_cast<std::uint64_t>(from), target);
      }
      return requiredBytes;
    } else if(from == 0) {
      if(availableBytes >= 1U) {
